...
const loadReferenceButton = document.getElementById("loadReferenceButton");
const referenceVideo = document.getElementById("referenceVideo");
const referenceCanvas = document.getElementById("referenceCanvas");
const referenceCtx = referenceCanvas.getContext("2d");
const referencePlayButton = document.getElementById("referencePlayButton");
const referencePauseButton = document.getElementById("referencePauseButton");
const referenceAnalyzeButton = document.getElementById(
  "referenceAnalyzeButton"
);
const referenceCurrentTime = document.getElementById("referenceCurrentTime");
const referenceTotalTime = document.getElementById("referenceTotalTime");
const referenceProgressBar = document.getElementById("referenceProgressBar");
...
const targetAnalyzeButton = document.getElementById("targetAnalyzeButton");
...
function calculateAllAngles(keypoints) {
  ...
  // 计算每个关键点相对于父节点的角度
  for (const [keypointName, parentName] of Object.entries(KEYPOINT_PARENTS)) {
    // 跳过根节点（没有父节点的关键点）
    if (parentName === null) {
...
function drawTrajectory(ctx, frameBuffer, keypointNames) {
  ...
  keypointNames.forEach((keypointName) => {
    ...
    frameBuffer.forEach((frame) => {
      const keypoint = frame.keypoints.find((kp) => kp.name === keypointName);
      if (keypoint && keypoint.score >= 0.2) {
    ...
    if (trajectoryPoints.length > 1) {
      ...
      // 如果有三个或更多点，使用贝塞尔曲线
        ...
        for (let i = 1; i < trajectoryPoints.length - 1; i++) {
          const xc = (trajectoryPoints[i].x + trajectoryPoints[i + 1].x) / 2;
          const yc = (trajectoryPoints[i].y + trajectoryPoints[i + 1].y) / 2;
...
function compareAngles(referenceAngles, targetAngles) {
  ...
  if (refShoulderDistances.length > 0) {
    const refSum = refShoulderDistances.reduce((acc, val) => acc + val, 0);
  ...
  if (targetShoulderDistances.length > 0) {
    ...
    avgTargetShoulderDistance = targetSum / targetShoulderDistances.length;
  }
  ...
  for (const keypointName in referenceAngles) {
    ...
    if (
      ...
      if (
        ...
        (keypointName.includes("wrist") || keypointName.includes("ankle")) &&
        referenceFrameBuffer.length > 0 &&
        targetFrameBuffer.length > 0
      ) {
        const refFrame = referenceFrameBuffer[referenceFrameBuffer.length - 1];
        const targetFrame = targetFrameBuffer[targetFrameBuffer.length - 1];
  ...
  // 计算平均差异
  const averageDifference = validCount > 0 ? totalDifference / validCount : 0;
...
function calculateUpperBodySimilarity(referenceAngles, targetAngles) {
  ...
  // 计算上半身相似度
  const similarity = validCount > 0 ? Math.max(0, 100 - totalDifference) : 0;
  return similarity;
}
  ...
  if (referenceKeypoints.length > 0 && targetKeypoints.length > 0) {
    ...
    if (windowComparison.similarity) {
      ...
      if (referenceVideo.currentTime >= beatInterval) {
        ...
        // 每拍记录一次最高相似度
        if (referenceVideo.currentTime - lastRecordedTime >= beatInterval) {
      ...
      // 计算并显示上半身和下半身瞬间相似度
      if (windowComparison.referenceAngles && windowComparison.targetAngles) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const targetWrapper = document.querySelector("#targetCanvas").parentElement;
  if (targetWrapper) {
